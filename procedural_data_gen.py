...
def generate_voxel_terrain(
    ...
    for x in range(width):
        for z in range(length):
            ...
            surface_y = height_scale * perlin_octaves(
                x * 0.1, 0, z * 0.1, octaves, persistence, lacunarity
            )
            for y in range(
                int(surface_y), int(surface_y) - depth, -1
            ):  # Extend from the surface down to 'depth'
